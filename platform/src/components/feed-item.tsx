...
export function FeedItem({ item, rank }: FeedItemProps) {
  ...
  return (
    <button
      type="button"
      className="w-full text-left rounded-lg border p-3 md:p-4 hover:shadow-sm transition cursor-pointer flex gap-3"
      data-item-id={item.id}
      onClick={handleCardClick}
    >
      {rank != null && (
        <span className="shrink-0 w-6 text-right text-sm tabular-nums text-muted-foreground/60 pt-0.5">
          {rank}.
        </span>
      )}
