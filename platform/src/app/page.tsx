...
import type { Content, FollowWithOrder, User } from "@/lib/types";
...
type MobileTab = "feed" | "people";

interface FeedColumnProps {
  linkSearch: string;
  setLinkSearch: (v: string) => void;
  setDebouncedLinkSearch: (v: string) => void;
  ...
  return (
    <>
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-auto space-y-2 md:space-y-3"
      >
        {feedLoading ? (
...
interface RightPanelProps {
  ...
  isLoadingSelectedUser: string | boolean | null;
  selectedUser: User | null;
  goBackToGlobal: () => void;
  rssUrl: string;
  degree: number;
  setDegreeParam: (d: number) => void;
  isInfoOpen: boolean;
  setIsInfoOpen: (v: boolean) => void;
  followsLoading: boolean;
  follows: FollowWithOrder[] | undefined;
  selectUser: (userLink: string) => void;
  userSearch: string;
  setUserSearch: (v: string) => void;
  isUserSearchPending: boolean;
      ...
      <div className="shrink-0 mb-3 md:mb-4">
        {userHandle ? (
          ...
          <div className="space-y-2">
            {/* Title row - hidden on mobile (shown above tabs) */}
            ...
            {/* RSS Subscribe Button - hidden on mobile */}
...
function HomeContent() {
  ...
  // Shared props for fragments
  const feedColumnProps: FeedColumnProps = {
    linkSearch,
    setLinkSearch,
    setDebouncedLinkSearch,
    feedContainerRef,
