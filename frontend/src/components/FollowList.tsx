import type { FollowWithOrder } from "@/lib/bindings/FollowWithOrder";
import type { FollowingUser } from "@/lib/bindings/FollowingUser";

type FollowListProps = {
  items: Array<FollowWithOrder>;
...
export function FollowList({ items, onSelect }: FollowListProps) {
  if (!items?.length) {
    return (
      <div className="text-sm text-muted-foreground">No follows for this degree.</div>
    );
  }
      ...
      {items.map((f) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">{String(f.order)}°</span>
          </li>
        )
      })}
    </ul>
